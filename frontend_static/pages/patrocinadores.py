        ...
        def guardar():
            ...
            try:
                if doc_id:
                    ...
                    nuevo["_id"] = generar_mongo_id("patrocinador", "patrocinador", nuevo["nombre"])
...
def page_patrocinadores():
    ...
    with ui.row().style("min-height:100vh; width:100%; gap:0;"):
        ...
        with ui.column().classes("flex-1").style("padding:24px; overflow-y:auto;"):
            ...
            """)
            tabla.on("relaciones", lambda e: mostrar_dialogo_relaciones("Patrocinador", e.args.get("_id"), e.args.get("nombre", "?")))
