...
def cargar_neo4j(driver, colecciones):
    ...
    relaciones = {
        ...
        ("Piloto", "PARTICIPA_EN", "Rally"): [
            {
                "origen": d["_id"],
                "destino": f'bulk_rally_{((int(d["_id"].split("_")[-1]) - 1) // 3) + 1:04d}',
            }
            for d in colecciones["pilotos"]
        ],
    ...
    # Elimina participaciones masivas obsoletas si cambio el criterio de
    # asignacion. Se limita a pilotos/copilotos y rallies de esta carga.
    for origen_label in ("Piloto", "Copiloto"):
        pares_validos = relaciones[(origen_label, "PARTICIPA_EN", "Rally")]
        driver.execute_query(
            f"""
            MATCH (a:{origen_label} {{origen_carga: 'masiva_1500'}})
                  -[r:PARTICIPA_EN]->
                  (b:Rally {{origen_carga: 'masiva_1500'}})
            WHERE NOT any(
                par IN $pares
                WHERE par.origen = a.mongo_id AND par.destino = b.mongo_id
            )
            DELETE r
            """,
            pares=pares_validos,
    ...
    # El modelo vigente no admite una relacion directa piloto-copiloto.
    driver.execute_query(
        """
        MATCH (p:Piloto {origen_carga: 'masiva_1500'})
              -[r:TIENE_COPILOTO]->
              (c:Copiloto {origen_carga: 'masiva_1500'})
        DELETE r
        """,
        database_="neo4j",
    )
...
def verificar(db, driver):
    ...
    if mongo_total != 1500 or neo_total != 1500 or rel_total != 2600:
        raise RuntimeError(
            "La carga no alcanzo los 1.500 documentos, 1.500 nodos y "
            "2.600 relaciones esperados"
        )
